  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form action="" @submit.prevent="addRecord">
      ...
      <div class="mb-6">
        ...
        <select id="artist" class="input select" v-model="newRecord.artist" required>
      ...
      <input type="submit" value="Add Record" class="font-sans font-bold px-4 rounded-lg cursor-pointer no-underline bg-green-500 hover:bg-green-700 block w-full py-4 text-white items-center justify-center" />
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        ...
        <div class="flex items-center justify-between flex-wrap">
          ...
          <button class="bg-transparent text-sm hover:bg-blue-500 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded-lg"
          @click.prevent="editRecord(record)">Edit</button>

          <button class="bg-transparent text-sm hover:bg-red-500 text-red-500 hover:text-white no-underline font-bold py-2 px-4 rounded-lg border border-red-500"
        ...
        <div v-if="record == editedRecord">
          <form action="" @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded-lg border border-grey-200 mt-4">

              <div class="mb-6">
                <label class="label">Title</label>
                <input type="text" class="input" v-model="record.title" required/>
              </div>

              <div class="mb-6">
                <label class="label">Year</label>
                <input type="number" class="input" v-model="record.year" required min="1000" max="3000"/>
              </div>

              <div class="mb-6">
                <label class="label">Artist</label>
                <select id="artist" class="input select" v-model="record.artist" required>
                  <option :value="artist.id" v-for="artist in artists" :key="artist.id">{{ artist.name }}</option>
                </select>
              </div>

              <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-500 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded-lg">
            </div>
          </form>
        </div>
      </li>
