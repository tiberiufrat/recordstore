<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form action="" @submit.prevent="addArtist">
      ...
      <input type="submit" value="Add Artist" class="font-sans font-bold px-4 rounded-lg cursor-pointer no-underline bg-green-500 hover:bg-green-700 block w-full py-4 text-white items-center justify-center" />
    </form>
    ...
    <ul class="list-reset mt-4">
      <li class="py-4" v-for="artist in artists" :key="artist.id" :artist="artist">
        ...
        <div class="flex items-center justify-between flex-wrap">
          ...
          <button class="bg-tranparent text-sm hover:bg-blue-500 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded-lg"
          @click.prevent="editArtist(artist)">Edit</button>

          <button class="bg-transprent text-sm hover:bg-red-500 text-red-500 hover:text-white no-underline font-bold py-2 px-4 rounded-lg border border-red-500"
        ...
        <div v-if="artist == editedArtist">
          <form action="" @submit.prevent="updateArtist(artist)">
            <div class="mb-6 p-4 bg-white rounded-lg border border-grey-light mt-4">
              <input type="text" class="input" v-model="artist.name" required />
              <input type="submit" value="Update" class=" my-2 bg-transparent text-sm hover:bg-blue-500 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 rounded-lg cursor-pointer">
            </div>
          </form>
        </div>
      </li>
